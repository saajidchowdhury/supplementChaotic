...
main = string(datetime('now','Format','M-d-y@HH.mm.ss'))+"code4025";
mkdir(main);
...
tmax = 2000e-6 / tau; %seconds -> au.
b0 = 0; %au
r0 = 20000; %au
umin = 0; umax = 1/2; ntheta = 100;
phimin = 0; phimax = pi/2; nphi = 100;
us = linspace(umin,umax,ntheta);
thetas = acos(1-2*us);
phis = linspace(phimin,phimax,nphi);
T = 1e-6; %Kelvin
mion = 170.936323 * amu; %u -> au
matom = 6.0151228874 * amu; %u -> au
...
vmin = 1e-09;
vmax = 1e-07;
vs = logspace(log10(vmin),log10(vmax),nv);
probabilities = zeros(nv,1);
...
for j = 1:nv
    ...
    parfor i = 1:ng
        ...
        arrayfun(@(theta,phi)g(theta,phi,v0), rthetasp{i}, rphisp{i});
    end
    tend = toc(tstart);
    ...
    probabilities(j) = length(bounces(bounces>=2))/length(bounces(:));
...
figure; hold on;
plot(vs,probabilities);
xlabel("Initial atom head-on velocity, $v_0$ (a.u.)");
ylabel("Probability of complex formation");
saveas(gcf,main+"/prob.png");
hold off; 

save(main+sprintf("/fig4trapq%.3f.mat",qx));



%wherever it says ode, compute the f(t,y) function
function [angle,bounces,lifetime,position,transfer,dist,nsteps,KE] = g(theta0,phi0,v0)

r0 = 2.00000000000000000000e+04;
tfinal = 8.26827466703403125000e+13;
rtol = 1.00000000000000003643e-10;
atol = 9.99999999999999945153e-21;
C4 = 8.20562999999999931333e+01;
C8 = 5.31546970412479996681e+08;
mion = 3.11597855084210925270e+05;
matom = 1.09648982550173714117e+04;
ax = -2.98199999999999984177e-04;
az = 5.96399999999999968353e-04;
qx = 2.50000000000000013878e-02;
OmegaRF = 3.79957461514366392655e-10;
rbounce = 5.99949821065623112304e+01;
